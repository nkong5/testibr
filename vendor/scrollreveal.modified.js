...
;(function () {
  ...
  function _configure (elem, config, container) {
    ...
    if (elem.config.origin === 'top' || elem.config.origin === 'bottom') {
      elem.config.axis = 'Y'
    } else if (elem.config.origin === 'left' || elem.config.origin === 'right') {
      elem.config.axis = 'X'
    } else {
	  elem.config.axis = "C"
	}
  ...
  function _generateTransform (elem) {
    ...
    if (parseInt(config.distance)) {
	  if (config.axis == "C") { 
		transform.initial += ' translate3D(' + cssDistance + ',' + cssDistance + ',0)';
		transform.target += ' translate3D(0,0,0)';
	  } else {
